...
def mock_graph() -> MagicMock:
    ...
    mock.invoke.return_value = {
        "messages": [
            MagicMock(content="Mocked response", response_metadata={"token_usage": {"prompt_tokens": 10, "completion_tokens": 20}})
        ]
    }
    return mock
...
def patch_build_graph(mock_graph: MagicMock):
    ...
    with patch("src.agents.graph.build_graph", return_value=mock_graph), \
...
def patch_config_manager(mock_config_manager: MagicMock):
    ...
    with patch("src.config.configuration.ConfigurationManager", return_value=mock_config_manager):
